...
public static class FormCanvasHtml
{
    ...
    /// <summary>HTML de impresion del valor Canvas: una hoja por pagina, con salto de pagina y "Pagina X de N"
    /// (el contador solo si hay &gt;1 pagina). Cadena vacia si el valor no es un Canvas valido.</summary>
    public static string Render(string? value)
    {
        ...
        for (var i = 0; i < pages.Count; i++)
        {
            ...
            if (pages.Count > 1)
            {
                sb.Append("<div class=\"dfr-canvas-pageno\" style=\"text-align:right;font-size:11px;color:#555;margin-top:2px;\">Pagina ")
                  .Append(i + 1).Append(" de ").Append(pages.Count).Append("</div>");
            }
            sb.Append("</div>");
        }
        return sb.ToString();
